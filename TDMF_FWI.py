...
class UnifiedMSSTB(nn.Module):
    """
    统一的多尺度仿射调制模块。
    支持任意目标尺寸（如 9, 18, 35, 35），自动将 70x70 低频先验下采样对齐。

    公式:
        x_out = γ ⊙ x_in + β + x_in
        其中 γ, β 由 adaptive_avg_pool2d(x_low, target_size) 生成
    """

    def __init__(self, low_channels, main_channels):
        super().__init__()

        self.low_channels = low_channels
        self.main_channels = main_channels
        # 调制网络：从低频先验生成 gamma 和 beta
        self.modulator = nn.Sequential(
            nn.Conv2d(low_channels, self.main_channels, kernel_size=3, padding=1),
            nn.SiLU(inplace=True),
            nn.Conv2d(self.main_channels, 2*self.main_channels, kernel_size=3, padding=1)
        )
        # 初始化：使初始 γ≈0 → γ+1≈1, β≈0（等价于恒等变换）
        nn.init.zeros_(self.modulator[-1].weight)
        nn.init.zeros_(self.modulator[-1].bias)

    def forward(self, x_low, x_in):
        """
        Args:
            x_in: (B, C, H, W) —— 高频特征（任意 H=W ∈ {9,18,35,70}）
            x_low: (B, 5, 70, 70) —— 低频先验（固定 70x70）
        Returns:
            x_out: (B, C, H, W)
        """
        B, C, H, W = x_in.shape

        # Step 1: 将 70x70 低频先验自适应下采样到 (H, W)
        x_low_resized = F.adaptive_avg_pool2d(x_low, (H, W))  # (B, 5, H, W)

        # Step 2: 生成仿射参数
        params = self.modulator(x_low_resized)  # (B, 2*C, H, W)
        gamma, beta = torch.chunk(params, 2, dim=1)  # each: (B, C, H, W)

        # Step 3: 仿射调制 + 残差
        x_out = gamma * x_in + beta + x_in

        return x_out

class WMSA_G1(nn.Module):
    # 在WMSA_G的基础上  优化了计算量 参考了LRSA 高斯部分的计算也优化了
    def __init__(self, input_dim=5, output_dim=5, head_dim=5, window_size=7, ratio=15, type='W'):
        super(WMSA_G1, self).__init__()
        ...
        self.q_proj = nn.Linear(input_dim, input_dim, bias=True)
        self.kv_proj = nn.Linear(input_dim, input_dim * 2, bias=True)
    ...
    def kv_pooling(self, x):
        """
        x: (B, N_win, N_token=49, C)
        return: (B, N_win, P, C), P = (ceil(W / s1) * ceil(W / s2)) ...
        """
        B, N_win, Nt, C = x.shape
        W = self.window_size
        x = x.view(B * N_win, W, W, C).permute(0, 3, 1, 2)  # (B*Nw, C, W, W)

        for pool_size in self.kv_pool_sizes:
            x = F.avg_pool2d(x, kernel_size=pool_size, stride=pool_size)

        _, C_out, H_p, W_p = x.shape
        return x.permute(0, 2, 3, 1).view(B, N_win, -1, C_out)

    def forward(self, x):  # x.shape: (1, 1050, 70, 5)
        if self.type != 'W':
        ...
        # self.kv_proj = nn.Linear(input_dim, input_dim * 2, bias=True)
        # Q: 保持原始 token
        q = self.q_proj(x)  # (1, 1500, 49, 5)

        # K, V: 先池化再投影
        kv_low = self.kv_pooling(x)  # (1, 1500, 9, 5)
        kv = self.kv_proj(kv_low)  # (1, 1500, 9, 10)
        k, v = kv.chunk(2, dim=-1)  # (1, 1500, 9, 5)

        # 多头 reshape
        q = rearrange(q, 'b nw n (h d) -> b h nw n d', h=self.n_heads) # (1, 1, 1500, 49, 5)
        k = rearrange(k, 'b nw n (h d) -> b h nw n d', h=self.n_heads) # (1, 1, 1500, 9, 5)
        v = rearrange(v, 'b nw n (h d) -> b h nw n d', h=self.n_heads) # (1, 1, 1500, 9, 5)

        # 计算权重
        sim = torch.einsum('b h w i d, b h w j d -> b h w i j', q, k) * self.scale  # (B, H, Nw, 49, P)
        # (1, 1, 1500, 49, 5)

        # 相对位置偏置：只加在 Q 维度（因 K 是池化的，无精确位置）
        rel_bias = rearrange(self.relative_embedding(), 'h i j -> h 1 1 i j')  # (1, 1, 1, 49, 49)
        sim = sim + rel_bias[..., :sim.size(-2), :sim.size(-1)]  # (1, 1, 1500, 49, 9) # 安全切片

        # 高斯先验：插值对齐 (49,49) → (49,P)
        prior = F.interpolate(
            self.global_attention_prior.expand(sim.shape[0], -1, -1, -1),
            size=(sim.shape[-2], sim.shape[-1]),
            mode='bilinear'
        ) # (1, 1, 49, 9)
        sim = sim + prior.unsqueeze(1)

        if self.type != 'W':
            attn_mask = self.generate_mask(h_windows, w_windows, self.window_size, shift=self.window_size // 2)
            sim = sim.masked_fill_(attn_mask, float("-inf"))

        probs = nn.functional.softmax(sim, dim=-1)  # (1, 1, 1500, 49, 9)
        output = torch.einsum('b h w i j, b h w j c -> b h w i c', probs, v) # (1, 1500, 49, 5)
        output = rearrange(output, 'b h w p c -> b w p (h c)')# (1, 1500, 49, 5)
...
class Block(nn.Module):
    def __init__(self, input_dim, output_dim, head_dim, window_size, drop_path, ratio, type='W', input_resolution=None):
        ...
        self.msa = WMSA_G1(input_dim, input_dim, head_dim, window_size, self.ratio, self.type)
        # self.msa = WMSA_G1(input_dim=5, input_dim=5, head_dim=5, window_size=7, self.ratio=15, self.type='W')
...
class TransBlock(nn.Module):
    def __init__(self, trans_dim=1, head_dim=1, window_size=7, drop_path=0.1, ratio=15, type='W', input_resolution=1):
        ...
        # self.trans_block = Block(self.trans_dim=5, self.trans_dim=5, self.head_dim=5, self.window_size=7, self.drop_path=0.1, self.ratio=15, self.type='W', self.input_resolution=70)
    ...
    def forward(self, x):
        ...
        trans_x = Rearrange('DataSplite c h w -> DataSplite h w c')(x_padded)   # 1, 1050, 70, 5
...
class TDMF_FWI(nn.Module):
    ...
    ## 简化了低频表达（去掉了多分支），利用仿射变换指导低频对高频的重建（UnifiedMSSTB），指导低频包括了9，
    # 完成 Low-Resolution Self-Attention for Semantic Segmentation  降低 WMSA_G计算量
    # 速度模型在[0,1]收敛时会出现震荡，归一化到[-1,1]反而出现较大误差  最后一层tanh优于sigmoid 收敛速度加快了许多，性能下降很小
    def __init__(self):
        ...
        self.trans1 = TransBlock(trans_dim=5, head_dim=5, window_size=7, drop_path=0.1, ratio=15, type='W',
                                 input_resolution=70)
        self.trans_9 = UnifiedMSSTB(75, 256)
        self.trans_18 = UnifiedMSSTB(75, 128)
        self.trans_35 = UnifiedMSSTB(75, 64)
        self.trans_70 = UnifiedMSSTB(75, 32)
    ...
    def forward(self, x):
        ...
        trans1 = self.trans1(x)  # (none, 75, 70, 70)  低频指导高频重建

        spatioUp4 = self.spatioUp4(spatioDown4, spatioDown5)  # (none, 256, 9, 9)

        trans_9 = self.trans_9(trans1, spatioUp4)   # (none, 256, 9, 9)
        spatioUp3 = self.spatioUp3(spatioDown3, trans_9)  # (none, 128, 18, 18)

        trans_18 = self.trans_18(trans1, spatioUp3) # (none, 128, 18, 18)
        spatioUp2 = self.spatioUp2(spatioDown2, trans_18)  # (none, 64, 35, 35)

        trans_35 = self.trans_35(trans1, spatioUp2)
        spatioUp1 = self.spatioUp1(temporal2spatio1, trans_35)  # (none, 32, 70, 70)

        trans_70 = self.trans_70(trans1, spatioUp1)
        outputs = self.last_layer(trans_70)

        #return trans1, spatioUp4, spatioUp3, spatioUp2, spatioUp1
...
if __name__ == '__main__':
    ...
    # 假设的 70×70 索引表
    x = torch.randn(10, 5, 1000, 70)  # 可以是一些预定义的值，或者根据实际情况进行学习
